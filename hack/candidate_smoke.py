...
def run_smoke(directory):
    with Candidate(directory, diagnostic_filename="smoke-failure.json") as deployed:
        ...
        enrollment=dict(deviceId="device-1",password="A"*43,channel="mdm")
        status,receipt=browser.call("POST","/api/v2/enrollments",enrollment,operation)
        require(status==200 and receipt["status"]=="pending","candidate enrollment failed")
        require(browser.call("POST","/api/v2/enrollments",enrollment,operation)==(status,receipt),"candidate replay changed enrollment")
        require(browser.call("GET","/api/v2/enrollments/"+receipt["enrollmentId"])[1]["status"]=="pending","candidate enrollment query failed")
        denied=str(uuid.uuid4())
        require(browser.call("POST","/api/v2/enrollments",dict(deviceId="outside",password="A"*43,channel="mdm"),denied)[0]==403,"device scope bypass")
