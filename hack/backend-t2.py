...
CONSUMERS={
 'policy':{'fact_pages_preserve_boundaries_and_reject_foreign_documents','persistence_replay_aba_and_old_facts','concurrent_cas_borrowed_rollback_and_runtime_owner','outbox_failure_and_immutable_inputs'},
 'resource':{'resource_admission_rejects_schema_and_privilege_drift','resource_immutable_versions_restart_and_reference_rollback','resource_cas_events_and_owner_admission'},
...
def fixture(source=ROOT,write_catalogs=False,app=False,migrations=None):
    with tempfile.TemporaryDirectory(prefix='mdm-backend-pg-') as directory:
        ...
        try:
            ...
            sql((source/'crates/app/schema/software-publication-roles.sql').read_text())
