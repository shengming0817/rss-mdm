...
def run(args, **kwargs):
    ...
    if result.returncode:
        ...
        raise RuntimeError('fixture command failed: ' + Path(args[0]).name + ' (exit ' + str(result.returncode) + ')')
...
def fixture(c):
    ...
    with tempfile.TemporaryDirectory(prefix=name+'-') as directory:
        ...
        public_port,private_port,pg_port=port(),port(),port()
        ...
        config={'format_version':1,'identity_origin':identity_origin,'database':database,'storage':storage,'listen':'0.0.0.0:8080','public_gateway':ips['public'],'private_gateway':ips['private'],'budgets':{'request_seconds':10,'drain_seconds':30,'resource_seconds':10},
            ...
            'hydra':{'admin_url':'https://hydra-admin:8443','addresses':[ips['hydra']+'/32'],'ca_file':PREFIX+'ca.crt','service_secret_file':PREFIX+'hydra-service','request_seconds':300,'code_seconds':60,'access_token_seconds':300,'clock_skew_seconds':30,'clients':[{'tenant_id':TENANT,'client_id':'mdm','audience':'mdm-api','config_version':1,'validation_secret_ref':'mdm-validation@1','validation_secret_file':PREFIX+'validation','oidc_secret_file':PREFIX+'oidc-client'}]}}
        config['hydra']['clients'].append({'tenant_id':TENANT,'client_id':'mdm-other','audience':'other-api','config_version':1,'validation_secret_ref':'other-validation@1','validation_secret_file':PREFIX+'validation-other','oidc_secret_file':PREFIX+'oidc-other'})
        ...
        def nginx(servers, extra=''):return 'pid /tmp/nginx.pid; error_log stderr crit; events {} http { access_log off; error_log stderr crit; client_body_temp_path /tmp/client; proxy_temp_path /tmp/proxy; proxy_next_upstream off; '+extra+servers+'}'
        ...
        try:
            ...
            docker('run','-d','--name',pg,'--label','rss.test=2343','--network',network,'--ip',ips['pg'],'--network-alias','pg','-p',f'127.0.0.1:{pg_port}:5432','-v',str(root)+':/fixture:ro',c['providers']['postgres'],'sh','-ec',pg_stage)
            ...
            app_run(c['images']['gateway'],['nginx','-e','stderr','-c',PREFIX+'public.conf','-g','daemon off;'],suffix='public',ip=ips['public'],extra=['-p',f'127.0.0.1:{public_port}:8443'])
            private_container=app_run(c['images']['gateway'],['nginx','-e','stderr','-c',PREFIX+'private.conf','-g','daemon off;'],suffix='private',ip=ips['private'],extra=['-p',f'127.0.0.1:{private_port}:8443'])
            binary=build_binary()
            ...
            if failures:raise RuntimeError('owned fixture cleanup failed')

def main():
    c=candidate()
    with fixture(c) as env:
        output=run(['cargo','test','--locked','-p','rss-mdm-app','--test','identity_t2','--','--ignored','--nocapture'],cwd=ROOT,env=env)
