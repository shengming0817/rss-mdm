...
当前接入边界见 [#2343 指南](../guides/202609091600-2343-mdm-identity.md)：首期单租户、静态角色、真实资产查询与危险动作授权拒绝。危险动作有权时仍返回不支持，不表示命令执行已交付；管理员注册许可与持久审计见 [F02 指南](../guides/202609090001-2347-enrollment-audit.md)：同一管理员签发/消费单次有界许可、申请受理与成功审计同事务，查询/拒绝审计失败不放行。申请受理不代表设备身份或证书已建立，运行证据由 #2347 PR 绑定。
