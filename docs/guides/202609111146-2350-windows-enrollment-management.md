...
`make t2` 使用真实 PG、真实 RSA 签名和原生 Rust TLS 监听器，覆盖未绑定叶证书拒绝、响应丢失、保存失败、提交未知/取消、重启、口令轮换、期限/取消竞争、并发换代、审计失败、错误用途/期限、错租户/设备、精确重放、nonce 跨会话和撤销。实际 Identity 候选组合另由 `make t2-identity` 验证，完整本地门禁为提交后的 `make ci`。具体测试机不是本次实施前置。
