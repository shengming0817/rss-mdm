...
汇总的 `assetStates` 是匹配设备的资产字段状态计数，分母不是设备数；`matched`、`unknown`、`total` 分别统计匹配、未知和授权候选设备。当前没有设备健康或合规评估 owner，接口不输出推测的合规结论。采集质量记录显式包含 source/channel、registration/generation/epoch。合法数据超过候选、来源或结果字节预算时返回 503；内部安全诊断分别记录 asset_object_limit、asset_source_limit、asset_bytes_limit，区别于输入错误 400，不截断数据。
