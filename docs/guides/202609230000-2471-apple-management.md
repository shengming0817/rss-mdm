...
设备叶证书的起止时间由验签结果持久化。服务在证书剩余寿命不足三分之一（最多提前七日）时，冻结当前 registration/generation，生成一次性 SCEP challenge，并经原生 InstallProfile 替换同 identifier、同顶层 UUID 的注册 profile。Topic、ServerURL、CheckInURL、权限和 capabilities 保持原值，SCEP payload 与身份引用使用新的 UUID，设备生成新密钥。此流程由设备身份 owner 管理，不产生业务 Commands 的成功事实。

签发通知只保存候选证书。新证书首次通过 mTLS、配置身份与 UDID 校验后，事务内替换活动 credential ID，并停用旧凭据与旧 SCEP attempt；registration、generation、source/epoch 保持不变。已认证但尚未提交的旧主体也会被当前 credential ID 检查拒绝。通知丢失可由新证书首次连接恢复；challenge 不允许重复消费或复用旧公钥。

续期授权最多一小时且不超过旧证书到期时间。设备没有证明新身份时保留旧身份；授权到期后生成新的 challenge/attempt，旧候选不能再激活。NotNow 复用同一请求和 CommandUUID。证书已经过期、注册已撤销、来源已停用或代际已更换时不能续期；过期且离线的设备须通过正常人工重新注册恢复，不降低 mTLS 校验。

`apple_identity_health` 在状态变化时记录 `valid`、`renewal_due`、`expired`，含 registration 和到期时间；`apple_identity_renewal` 记录准备的 attempt。日志不包含 challenge、token、CSR 或证书内容。真实 macOS 对更新 profile 的行为仍由 #2482 的 T3 验证；受控 T2 证明协议与服务端生命周期。
