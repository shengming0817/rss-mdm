...
跨 Scope→Group 的引用保护由 N12 组装持有：删除被引用组或直接目标必须拒绝；先显式解除引用后才能删除，不静默级联清空范围。新增引用、解除引用与删除必须在同一受控事务/锁定顺序下串行校验，防止检查后新增引用；N09 提供参与该事务的受控写入接缝；standalone execute 拒绝删除，借用入口先通过 RSS 的不可伪造 runtime owner 校验及 tenant 校验。N09 不反向依赖 Scope 或读取其业务表。被计划引用的 Scope 版本同样禁止静默删除。
...
两核心采用各自拥有的角色化 ID，组/设备、策略/载荷/请求/目标快照不能在公共签名中互换；不保留通用 ObjectKey 路径。
Policy 暂停只关闭 Apply 调度并保留当前版本身份，恢复沿用原版本；明确范围退出和归档按显式移除规则产生取消意图，
新版本取代时旧非终态也产生取消意图，包括新版本激活后立即暂停的情况。当前仅接纳停止执行并保留已有效果，不生成 cleanup 载荷计划。
...
N08 的 [发布核心](../../../crates/software-release/README.md) 使用整体隔离/弃用状态与逐环携证据状态，
Test/Pilot/Production 每环重新验证审批；验证不得早于当前内容生效时间或前环发布成功时间，恢复同样校验。
默认发布者与审批者分离，调用方可显式允许同人。
首次发布授权后内容冻结；未知结果按原发布身份对账，确认未生效才显式重试。
本期隔离/弃用无解除；迟到成功仅补充发布事实，不重新开放授权。请求重放只返回历史回执。
...
- 字段定义包含键、类型、单位及可用操作；值为字符串、布尔、整数、UTC 时间或同类型集合，不隐式字符串转数值/时间。事实携带来源、快照身份和采集时间信息（不包含字段有效期）；合法 Null 与 Missing/Deleted/Conflict/Unsupported 分开，错误字段与无权限字段不能静默忽略。当前字段与统一消费接线见 [#2463 资产指南](../../guides/202609210000-2463-unified-assets.md)，Inventory 持有唯一目录，应用适配到 Group；字段无过期或未来生效窗口。
...
MDM 业务控制台暂不重写；实际 WinMDM 前端当前未取得，rss-web 的 apps/web 属 RSS 浏览器客户端，不能用它替代 MDM 业务控制台。#2368/#2364 明确复用其独立 apps/identity 认证页面，MDM 拥有同源宿主装配、资源授权和产品验收；此复用不扩大为 MDM 控制台。认证 UI 与服务端镜像由同一产品 candidate.json 绑定不可变身份和恢复归档，浏览器工具为独立验收输入，不进入生产依赖。先取得仓库 revision、API/错误/分页/会话契约再冻结兼容。本批 Group 直接采用上述类型化契约；旧 Criteria/expr/SQL 仅提供行为对照，不承诺表达式向后兼容，不支持的旧规则明确拒绝。存量规则盘点与显式重建归 M01，不在核心保留旧解释器或自动转换路径。
