...
- 字段定义包含键、类型、单位及可用操作；值为字符串、布尔、整数、UTC 时间或同类型集合，不隐式字符串转数值/时间。事实携带来源、快照身份和采集时间信息（不包含字段有效期）；合法 Null 与 Missing/Stale/Unsupported 分开，错误字段与无权限字段不能静默忽略。当前字段与统一消费接线见 [#2463 资产指南](../../guides/202609210000-2463-unified-assets.md)，Inventory 持有唯一目录，应用适配到 Group；字段无过期或未来生效窗口。
