...
WITH relations AS (
 ...
 WHERE c.relkind='r' AND (n.nspname IN('rss_device_command','rss_reconcile','mdm_apple') OR (n.nspname='mdm_access' AND c.relname IN
 ('requests','devices','registrations','credentials','report_sources','enrollment_intents','enrollment_certificates','authorization_rules','user_groups','management_sessions','management_messages','collection_runs','audit')))
), contracts AS (
