...
SELECT current_user='mdm_command_runtime' AND session_user=current_user
 ...
 AND (SELECT array_agg(relname::text ORDER BY relname)=ARRAY['action_attempts','action_plans','action_polls','action_receipts','action_runs','attempt_history','attempts','capabilities','capability_queries','devices','firewall_owners','operations','plan_executions','requests'] FROM tables)
