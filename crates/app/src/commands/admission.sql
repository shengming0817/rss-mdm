WITH tables AS (
 ...
 ('mdm_commands.action_plans',true,true,false),('mdm_commands.action_runs',true,true,false),('mdm_commands.action_receipts',true,true,false),('mdm_commands.action_attempts',true,true,false),('mdm_resource.aggregates',true,false,false),('mdm_resource.immutable',true,false,false),('mdm_access.agent_bindings',true,false,false),
...
SELECT current_user='mdm_command_runtime' AND session_user=current_user
 ...
 AND (SELECT array_agg(relname::text ORDER BY relname)=ARRAY['action_attempts','action_plans','action_receipts','action_runs','attempt_history','attempts','capabilities','capability_queries','devices','firewall_owners','operations','plan_executions','requests'] FROM tables)
