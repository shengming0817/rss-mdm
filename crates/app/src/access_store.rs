...
"#).fetch_one(&mut *tx).await.map_err(db)?;
    ...
    if policies != 17 || retention != 2 || total != 19 {
        return Err(Error::Unavailable(Failure::AccessAdmission));
    }
    tx.rollback().await.map_err(db)
}
