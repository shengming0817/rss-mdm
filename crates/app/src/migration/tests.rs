...
async fn fresh_installation_replay_and_mismatch_rejection() -> Result<()> {
    for (database, baseline_units) in [("mdm_installation", 39), ("mdm_installation_tasks", 40)] {
        ...
        let candidate = units();
        // Exercise both accepted exact baselines against the merged migration chain.
        migrate_units(&mut owner, &installation, &candidate[..baseline_units]).await?;
        sqlx::query("SELECT set_config('rss.tenant_id',$1,false)")
            .bind(&installation.tenants[0])
            .execute(&mut owner)
            .await?;
        let fixture = include_str!("agent-v1-fixture.sql");
        let fixture = if baseline_units == 40 {
            fixture
                .replace("grant_id,channel)", "grant_id,source)")
                .replace(
                    "66666666-6666-4666-8666-666666666666','agent');",
                    "66666666-6666-4666-8666-666666666666','agent.builtin');",
                )
        } else {
            fixture.to_owned()
        };
        // Only fixed test fixture literals are substituted; no external SQL input.
        sqlx::raw_sql(sqlx::AssertSqlSafe(fixture.as_str()))
            .execute(&mut owner)
            .await?;
        owner
            .execute("SELECT set_config('rss.tenant_id','',false)")
            .await?;
        let mut shuffled: Vec<(String, String, bool)> = sqlx::query_as(
            "SELECT name,digest,complete FROM public.mdm_migrations ORDER BY name DESC",
        )
        .fetch_all(&mut owner)
        .await?;
        if shuffled
        ...
        sqlx::query_scalar::<_, bool>(
            "SELECT NOT EXISTS(SELECT 1 FROM public.mdm_migrations WHERE name='agent-access-v2')"
        )
        .fetch_one(&mut owner)
        .await?
    );
        ...
        sqlx::query_scalar("SELECT to_jsonb(r) - 'apple_approval' - 'apple_deadline' FROM mdm_access.collection_runs r")
            .fetch_one(&mut owner)
            .await?;
        owner
            .execute("SELECT set_config('rss.tenant_id','',false)")
        ...
        sqlx::query_scalar::<_, serde_json::Value>(
            "SELECT to_jsonb(r) - 'apple_approval' - 'apple_deadline' FROM mdm_access.collection_runs r"
        )
        .fetch_one(&mut owner)
        .await?
            == historical
...
fn task_upgrade_accepts_only_the_exact_named_baseline_or_complete_current_ledger() {
    ...
    assert!(accepted_ledger(&ledger(&current[..40]), &current));
    assert!(!accepted_ledger(&ledger(&current[..41]), &current));
...
#[tokio::test]
#[ignore = "make t2: Apple upgrade preflight and preserved Windows history"]
async fn fresh_installation_replay_and_mismatch_rejection_apple() -> Result<()> {
    let options = PgConnectOptions::from_str(&std::env::var("MDM_OWNER_URL")?)?
        .database("mdm_installation_apple")
        .ssl_mode(sqlx::postgres::PgSslMode::VerifyFull)
        .ssl_root_cert(std::env::var("PG_CA_FILE")?);
    let installation = Installation {
        instance_id: crate::identity_fixture::INSTANCE.into(),
