    ...
    async fn management(
        ...
        if let Some(row) = &stored {
            if !row.try_get::<bool, _>("live").map_err(db)?
                ...
                || row.try_get::<String, _>("credential").map_err(db)?
        ...
        let authenticated_before = stored
            ...
            .transpose()?
            .unwrap_or(false);
        let authenticated = match &message.header.credential {
            Some(credential) => {
            ...
            None => authenticated_before,
        };
        let mut nonce: Vec<u8> = stored
            .as_ref()
        ...
        let complete = authenticated && server_authenticated;
