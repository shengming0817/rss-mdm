            ...
            let rows=tx.with_connection(move |c|Box::pin(async move {
                sqlx::query(r#"
                  ...
                  AND EXISTS(SELECT 1 FROM mdm.asset_changes c WHERE c.tenant_id=g.tenant_id AND c.revision>$2 AND c.revision<=$3 AND (
                    ...
                    OR (g.kind='static' AND c.kind IN('device','registration','source','credential') AND EXISTS(SELECT 1 FROM mdm_group.member_rows m WHERE m.tenant_id=g.tenant_id AND m.run_id=g.member_set AND m.matched AND m.object_id=c.identity->>'device'))
