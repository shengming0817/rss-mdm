...
CREATE TABLE mdm_apple.attempts (
 ...
 operation uuid, collection uuid, phase text NOT NULL CHECK(phase IN ('collect','execute','observe')),
 ...
 CHECK((phase='collect' AND collection IS NOT NULL AND operation IS NULL) OR (phase IN ('execute','observe') AND operation IS NOT NULL AND collection IS NULL)),
