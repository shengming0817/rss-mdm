...
DO $$ DECLARE t text; BEGIN
 ...
 OR EXISTS(SELECT 1 FROM rss_transactional_messaging.outbox WHERE domain='mdm.commands.v1' AND status<>'published') THEN
 RAISE EXCEPTION 'finish or cancel old commands and settle their dispatch before upgrade';
 END IF;
...
CREATE TABLE mdm_commands.attempts (
 ...
 status integer, value text, received_at bigint, request bytea NOT NULL,
...
CREATE TABLE mdm_commands.plan_executions (
 ...
 PRIMARY KEY(tenant_id,plan), FOREIGN KEY(tenant_id,plan) REFERENCES mdm_management.previews(tenant_id,id)
);
DO $$ DECLARE n text; t text; BEGIN
 FOR n,t IN SELECT * FROM (VALUES ('mdm_commands','attempts'),('mdm_commands','attempt_history'),('mdm_commands','capabilities'),('mdm_commands','capability_queries'),('mdm_commands','plan_executions'),('mdm_management','firewall_resources'),('mdm_management','firewall_versions')) AS names(n,t) LOOP
...
GRANT UPDATE(status,value,received_at) ON mdm_commands.attempts TO mdm_command_runtime;
