...
ALTER TABLE mdm_access.collection_runs ADD CONSTRAINT collection_source_profile CHECK(
 ...
 OR (source IN ('agent.builtin','agent.script','agent.osquery') AND session_id IS NULL AND request_message IS NULL AND first_command IS NULL AND request IS NULL AND sealed_at IS NOT NULL AND result<>'pending' AND reason='complete' AND batch IS NOT NULL AND apple_approval IS NULL AND apple_deadline IS NULL)
