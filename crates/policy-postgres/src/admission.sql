WITH tables AS (
 ...
 WHERE n.nspname='mdm_policy' AND c.relkind='r'
), reachable AS (
 SELECT * FROM pg_roles WHERE oid=(SELECT oid FROM pg_roles WHERE rolname=current_user)
 OR pg_has_role(current_user,oid,'MEMBER')
)
SELECT
 (SELECT array_agg(relname::text ORDER BY relname)=ARRAY['aggregates','facts','immutable','requests'] FROM tables)
 AND NOT EXISTS(SELECT 1 FROM pg_class c JOIN pg_namespace n ON n.oid=c.relnamespace
 WHERE n.nspname='mdm_policy' AND c.relkind NOT IN('r','i'))
 AND NOT EXISTS(SELECT 1 FROM pg_proc p JOIN pg_namespace n ON n.oid=p.pronamespace
 WHERE n.nspname='mdm_policy')
 AND NOT EXISTS(SELECT 1 FROM reachable WHERE rolsuper OR rolbypassrls OR rolcreaterole OR rolcreatedb OR rolreplication
 OR oid IN (SELECT relowner FROM tables) OR has_schema_privilege(oid,'mdm_policy','CREATE'))
 AND has_schema_privilege(current_user,'mdm_policy','USAGE')
 ...
 WHERE n.nspname='mdm_policy' AND (a.grantee=0 OR (a.grantee IN(SELECT oid FROM reachable) AND a.is_grantable)))
 ...
 AND (has_column_privilege(current_user,t.oid,a.attnum,'UPDATE') <>
  (t.relname='aggregates' AND a.attname IN('revision','document','digest') OR t.relname='facts' AND a.attname IN('document','digest'))
