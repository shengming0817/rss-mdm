...
`hack/core_consumer.py` 从固定产品 Git SHA 复用 `tests/model.rs`，仅直接消费本产品包及 canonical RSS 基础类型，
在仓外分别验证默认与关闭默认 features 的独立 lock、依赖闭包和实际行为；结果归本地 CI artifact。
